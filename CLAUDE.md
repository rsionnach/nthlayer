...
Active sibling repo (implemented):

- `nthlayer-override-adapter/` — override-event sidecar (opensrm-jmy.7 + jmy.18, implemented): HTTP → gen_ai.override OTel span bridge + sidecar→core verdict-binding POST. Own repo, own release-please, own Dockerfile. Console script: `nthlayer-override-adapter serve`. Depends on `nthlayer-common` overrides foundation (jmy.4 + jmy.11). Implementation plan: [docs/superpowers/plans/2026-05-15-jmy7-override-adapter-sidecar.md](docs/superpowers/plans/2026-05-15-jmy7-override-adapter-sidecar.md). Follow-ups: jmy.17 (Slack adapter), jmy.19 (bench post_incident mutation-style attribution).
...
- [Override Adapter Sidecar design](docs/superpowers/specs/2026-05-15-jmy7-override-adapter-sidecar-design.md) — opensrm-jmy.7: new `nthlayer-override-adapter/` repo (Wave A); generic-webhook → gen_ai.override OTel spans; batch with last-in-array-wins duplicate handling; privacy hashing at emission boundary. Follow-ups: jmy.17 (Slack adapter), jmy.19 (bench post_incident mutation-style attribution)
- [Override Adapter Sidecar implementation plan](docs/superpowers/plans/2026-05-15-jmy7-override-adapter-sidecar.md) — opensrm-jmy.7 task checklist (Tasks 0–12): bootstrap, config, metrics, emission, routes (canonical/batch/webhook), app, CLI, smoke tests, README/CLAUDE.md, Dockerfile + CI/release. Follow-ups: jmy.17 (Slack adapter), jmy.19 (bench post_incident mutation-style attribution)
- [Override Verdict-Binding Path design](docs/superpowers/specs/2026-05-20-jmy18-override-verdict-binding-design.md) — opensrm-jmy.18: reframed from OTel consumer to sidecar→core direct HTTP POST; new `POST /verdicts/{id}/override` endpoint in nthlayer-core (mutation-style, distinct from lineage-style `POST /outcome`); `CoreAPIClient.apply_override()` in nthlayer-common; `OverridePrivacyConfig.pre_redacted` flag (deprecates `plaintext_reviewer`); core-binding path in nthlayer-override-adapter (`core.{url,timeout_seconds}` config, `bind_to_core()` helper, `bindings` response field, `nthlayer_override_binding_total` Prometheus counter). OTel and core POST run sequentially but independently (fail-open on both). Privacy applied once at sidecar entry. Follow-up: jmy.19 (bench post_incident mutation-style attribution).
- [Override Verdict-Binding Path implementation plan](docs/superpowers/plans/2026-05-20-jmy18-override-verdict-binding.md) — opensrm-jmy.18 task checklist (Phases A/B/C): Phase A = nthlayer-common (pre_redacted flag, OverrideEvent.to_dict, CoreAPIClient.apply_override); Phase B = nthlayer-core (POST /verdicts/{id}/override handler + test_api_overrides.py); Phase C = nthlayer-override-adapter (CoreConfig, binding_total counter, BindingResult, bind_to_core, route wiring, CoreAPIClient lifecycle). Phase D = nthlayer cross-process smoke test.
...
- **0a — `.start.lock` concurrent-invocation mutex.** Lock dir at `$OUTPUT_DIR/.start.lock` holding `pid` (owning shell `${BASHPID:-$$}`) + `owner` (literal `demo.sh-start-lock`). `mkdir` is the atomic primitive (shell `mv` into an existing directory moves *into* it rather than failing, so an `mv`-rename dance does not give atomicity for directory names). After `mkdir` succeeds, pid + owner are written in two `echo`s; a concurrent reader uses `_read_lock_state` to poll up to ~200ms for the metadata to appear before making a decision (tolerates the establish window). Stale-lock reclaim requires the recorded PID to fail `kill -0` AND the owner tag to match the literal — protects against PID recycling pointing the cleanup logic at the user's IDE. If the 200ms poll completes with empty pid or owner, the caller refuses with a manual-cleanup hint rather than auto-reclaiming (false refusal costs one `rm -rf`; false reclaim under load would produce concurrent boots — the exact bug this guard prevents). Cleanup via the `_demo_cleanup` registry (single EXIT trap, composable hooks) so future cleanup-needing code can append rather than clobber. A `_DEMO_START_LOCK_ESTABLISHING` sentinel lets the cleanup hook also rm a half-built lock when SIGINT lands between `mkdir` and the metadata `echo`s. Refuses to run if `$OUTPUT_DIR` is a symlink.
