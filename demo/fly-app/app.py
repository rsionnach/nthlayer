...
# Basic Auth - Security validation
METRICS_USERNAME = os.environ.get("METRICS_USERNAME", "nthlayer")
METRICS_PASSWORD = os.environ.get("METRICS_PASSWORD")

# Validate password configuration at startup (fail-fast security)
if METRICS_PASSWORD is None:
    raise RuntimeError(
        "METRICS_PASSWORD environment variable must be set. "
        "This prevents accidental deployment without authentication."
    )

if METRICS_PASSWORD == "demo":
    raise RuntimeError(
        "METRICS_PASSWORD cannot be 'demo' - this default is insecure. "
...
http_in_flight = Gauge(
    "http_requests_in_flight", "Current in-flight requests", ["service"], registry=registry
)

# PostgreSQL metrics (payment-api, identity-service)
pg_connections = Gauge(
    "pg_stat_database_numbackends", "Active connections", ["service", "datname"], registry=registry
)
pg_max_conn = Gauge(
    "pg_settings_max_connections", "Max connections", ["service"], registry=registry
)
pg_blks_hit = Gauge(
    "pg_stat_database_blks_hit", "Buffer cache hits", ["service", "datname"], registry=registry
)
pg_blks_read = Gauge(
    "pg_stat_database_blks_read", "Disk reads", ["service", "datname"], registry=registry
)
...
pg_activity_count = Gauge(
    "pg_stat_activity_count", "Connections by state", ["service", "state"], registry=registry
)
...
pg_database_size = Gauge(
    "pg_database_size_bytes", "Database size", ["service", "datname"], registry=registry
)
pg_deadlocks = Gauge(
    "pg_stat_database_deadlocks", "Deadlocks", ["service", "datname"], registry=registry
)
pg_replication_lag = Gauge(
    "pg_replication_lag_seconds", "Replication lag", ["service"], registry=registry
)
...
redis_connections = Gauge(
    "redis_connected_clients", "Connected clients", ["service"], registry=registry
)
redis_memory = Gauge("redis_memory_used_bytes", "Memory used", ["service"], registry=registry)
redis_keys = Gauge("redis_db_keys", "Total keys", ["service", "db"], registry=registry)

# Cache metrics (payment-api uses these for Redis)
cache_hits = Counter("cache_hits_total", "Cache hits", ["service"], registry=registry)
cache_misses = Counter("cache_misses_total", "Cache misses", ["service"], registry=registry)

# Additional Redis metrics
redis_evicted_keys = Counter(
    "redis_evicted_keys_total",
    "Evicted keys due to memory pressure",
    ["service"],
    registry=registry,
)
redis_expired_keys = Counter(
    "redis_expired_keys_total", "Keys expired by TTL", ["service"], registry=registry
)
redis_commands = Counter(
    "redis_commands_processed_total", "Commands processed", ["service"], registry=registry
)

# Kubernetes metrics (payment-api, notification-worker, analytics-stream)
kube_pod_status = Gauge(
    "kube_pod_status_phase", "Pod status", ["service", "pod", "phase"], registry=registry
)
...
mysql_connections = Gauge(
    "mysql_global_status_threads_connected", "MySQL connections", ["service"], registry=registry
)
mysql_max_conn = Gauge(
    "mysql_global_variables_max_connections", "Max connections", ["service"], registry=registry
)
mysql_queries = Counter(
    "mysql_global_status_queries_total", "Total queries", ["service"], registry=registry
)

# RabbitMQ metrics (checkout-service)
rabbitmq_messages = Gauge(
    "rabbitmq_queue_messages", "Messages in queue", ["service", "queue"], registry=registry
)
rabbitmq_consumers = Gauge(
    "rabbitmq_queue_consumers", "Active consumers", ["service", "queue"], registry=registry
)
...
ecs_tasks = Gauge(
    "ecs_service_running_count", "Running tasks", ["service", "cluster"], registry=registry
)
ecs_cpu = Gauge(
    "ecs_task_cpu_utilization", "CPU utilization %", ["service", "task"], registry=registry
)
ecs_memory = Gauge(
    "ecs_task_memory_utilization", "Memory utilization %", ["service", "task"], registry=registry
)

# Notification worker metrics
notif_sent = Counter(
    "notifications_sent_total", "Notifications sent", ["service", "status"], registry=registry
)
...
kafka_lag = Gauge(
    "kafka_consumer_lag_seconds", "Consumer lag", ["service", "topic"], registry=registry
)
...
kafka_throughput = Gauge(
    "kafka_consumer_records_per_second", "Records/sec", ["service", "topic"], registry=registry
)
kafka_partitions = Gauge(
    "kafka_topic_partitions", "Number of partitions", ["service", "topic"], registry=registry
)

# Analytics stream metrics
events_processed = Counter(
    "events_processed_total", "Events processed", ["service", "status"], registry=registry
)
...
stream_throughput = Gauge(
    "stream_throughput_events_per_second", "Throughput", ["service"], registry=registry
)

# MongoDB metrics (analytics-stream)
mongo_connections = Gauge(
    "mongodb_connections", "Active connections", ["service", "state"], registry=registry
)
mongo_ops = Counter(
    "mongodb_operations_total", "Operations", ["service", "type"], registry=registry
)
...
login_attempts = Counter(
    "login_attempts_total", "Login attempts", ["service", "status"], registry=registry
)
user_registrations = Counter(
    "user_registrations_total", "User registrations", ["service"], registry=registry
)
password_resets = Counter("password_reset_total", "Password resets", ["service"], registry=registry)
...
es_index_size = Gauge(
    "elasticsearch_indices_store_size_bytes", "Index size", ["service", "index"], registry=registry
)
es_docs_count = Gauge(
    "elasticsearch_indices_docs", "Document count", ["service", "index"], registry=registry
)
es_jvm_heap_used = Gauge(
    "elasticsearch_jvm_memory_used_bytes", "JVM heap used", ["service", "area"], registry=registry
)
es_jvm_heap_max = Gauge(
    "elasticsearch_jvm_memory_max_bytes", "JVM heap max", ["service", "area"], registry=registry
)
es_gc_time = Counter(
    "elasticsearch_jvm_gc_collection_seconds_sum", "GC time", ["service", "gc"], registry=registry
)
...
def simulate_payment_api():
    ...
    pg_database_size.labels(service=svc, datname="payments").set(
    ...
    for table in ["users", "orders", "sessions", "audit_log"]:
        pg_dead_tuples.labels(service=svc, schemaname="public", relname=table).set(
            random.randint(100, 15000)
        )
        pg_index_scans.labels(
            service=svc, schemaname="public", relname=table, indexrelname=f"{table}_pkey"
        ).inc(random.randint(100, 500))
    ...
    for i in range(1, 4):
        ...
        container_cpu.labels(service=svc, pod=pod, container="payment-api").inc(
            random.uniform(0.01, 0.05)
        )
        container_memory.labels(service=svc, pod=pod, container="payment-api").set(
...
def simulate_notification_worker():
    ...
    for partition in range(3):
        kafka_offset.labels(service=svc, topic="notifications", partition=str(partition)).inc(
    ...
    for i in range(1, 4):
        ...
        kube_pod_status.labels(service=svc, pod=pod, phase="Running").set(1)
        container_cpu.labels(service=svc, pod=pod, container="worker").inc(
            random.uniform(0.02, 0.08)
        )
        container_memory.labels(service=svc, pod=pod, container="worker").set(
...
def simulate_analytics_stream():
    ...
    for partition in range(3):
        kafka_offset.labels(service=svc, topic="events", partition=str(partition)).inc(
    ...
    for i in range(1, 5):
        ...
        kube_pod_status.labels(service=svc, pod=pod, phase="Running").set(1)
        container_cpu.labels(service=svc, pod=pod, container="stream-processor").inc(
            random.uniform(0.05, 0.15)
        )
        container_memory.labels(service=svc, pod=pod, container="stream-processor").set(
...
def simulate_identity_service():
    ...
    pg_database_size.labels(service=svc, datname="identity").set(
        random.randint(2_000_000_000, 15_000_000_000)
    )
...
def simulate_search_api():
    ...
    for index in ["products", "users", "content"]:
        ...
        es_index_size.labels(service=svc, index=index).set(
    ...
    for i in range(1, 4):
        pod = f"search-api-{i}"
        kube_pod_status.labels(service=svc, pod=pod, phase="Running").set(1)
        container_cpu.labels(service=svc, pod=pod, container="search-api").inc(
            random.uniform(0.02, 0.1)
        )
        container_memory.labels(service=svc, pod=pod, container="search-api").set(
