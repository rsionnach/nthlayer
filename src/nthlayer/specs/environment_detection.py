    ...
    for var_name, extractor in ENV_VAR_MAPPINGS.items():
        ...
        if value:
            try:
                ...
                if env:
                    return env
            except Exception:  # intentionally ignored: skip invalid env var extractors
                continue

    return None


def get_environment(explicit_env: Optional[str] = None, auto_detect: bool = True) -> Optional[str]:
