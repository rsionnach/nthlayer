...
class RecordingRuleBuilder:
    ...
    def build(self) -> List[RecordingRuleGroup]:
        ...
        if self.slo_resources:
            ...
            slo_group = RecordingRuleGroup(
                name=f"{self.context.name}_slo_metrics",
                interval="30s"
            )
    ...
    def _build_availability_rules(self, service: str, slo_name: str, spec: dict) -> List[RecordingRule]:
    ...
    def _build_error_rate_rules(self, service: str, slo_name: str, spec: dict) -> List[RecordingRule]:
    ...
    def _build_health_rules(self) -> RecordingRuleGroup:
        ...
        group = RecordingRuleGroup(
            name=f"{service}_health_metrics",
            interval="30s"
        )
        
        # Request rate (requests per second)
        group.add_rule(RecordingRule(
...
def build_recording_rules(service_context: ServiceContext, resources: List[Resource]) -> List[RecordingRuleGroup]:
